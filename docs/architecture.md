...
1. `popup.js` restores the last selected data categories from `chrome.storage.local` and queries the active tab.
...
4. The extension calls `chrome.browsingData.remove` once per selected category, reporting per-step progress (e.g., "Removendo cache do navegador... (2/6)"), persists the selection for the next use, shows the result as soon as removal finishes, and reloads that tab afterwards.
